@php
    ...
    function checkQuestionProblems($qa)
    {
        $problems = [];
        ...
        if ($qa->category_id == 2 || $qa->category_id == 3) {
    ...
    foreach ($question as $qa) {
        if (count(checkQuestionProblems($qa)) > 0) {
    ...
    <div class="manage-page">
        ...
        <div class="card content-card">
            <div class="card-header">
                <h4 class="card-title">Round 1 Category-wise Question Distribution</h4>
        ...
        <div class="row">
            <div class="col-xl-8 col-lg-12">
                <div class="card content-card">
                    <div class="card-header">
                        <form id="questionUploadForm" action="{{ url('/add-question-from-excel') }}" method="post" enctype="multipart/form-data">
                            @csrf
                            <input type="hidden" name="force_upload" id="forceUploadExcel" value="0">
                            <div class="upload-panel">
                                <label for="questionExcelFile">Excel Upload</label>
                                <div class="d-flex flex-wrap align-items-center" style="gap: 10px;">
                                    <input class="form-control form-control-sm" id="questionExcelFile" type="file"
                                        accept=".csv,.xlsx,.xls" name="question_excel_file" style="max-width: 320px;">
                                    <button type="button" id="validateExcelButton" class="btn btn-sm btn-outline-primary">
                                        <i class="fa fa-check-circle mr-1"></i> Validate File
                                    </button>
                                    <button type="submit" id="uploadExcelButton" class="btn btn-sm btn-primary" disabled>
                                        <i class="fa fa-upload mr-1"></i> Upload
                                    </button>
                                    <button type="submit" id="uploadWithProblemsButton" class="btn btn-sm btn-warning" style="display: none;">
                                        <i class="fa fa-exclamation-triangle mr-1"></i> Upload With Problems
                                    </button>
                                    <a href="{{ route('question.export') }}" class="btn btn-sm btn-outline-success">
                                        <i class="fa fa-download mr-1"></i> Export Questions
                                    </a>
                                    <a href="{{ route('question.export.import.ready') }}" class="btn btn-sm btn-outline-info">
                                        <i class="fa fa-download mr-1"></i> Export Import Template
                                    </a>
                                </div>
                                <div id="excelValidationResult" class="mt-3">
                            ...
                            <table id="dataTable" class="table table-hover mb-0">
                                ...
                                <tbody>
                                    @forelse ($question as $qa)
                                        @php
                                            $problems = checkQuestionProblems($qa);
                                            $options = json_decode($qa->option, true);
                                            if (!is_array($options)) {
                                                $options = [];
                                            }
                                        ...
                                        <tr class="{{ count($problems) > 0 ? 'problem-row' : '' }}">
                                                ...
                                                @if ($qa->category_id == 3 || $qa->category_id == 2)
                                                    @if (!empty($qa->image_question))
                                                        <img class="question-img"
                                                            src="{{ asset('storage/question/' . $qa->image_question) }}"
                                                            alt="Question Image">
                                                    @else
                                            ...
                                            <td>
                                                <a class="btn btn-sm btn-warning btn-icon"
                                                    href="{{ route('question.edit', $qa->id) }}">
                                                    <i class="fa fa-edit" aria-hidden="true"></i>
                                                </a>

                                                <a class="btn btn-sm btn-danger btn-icon delete-form"
                                                    href="{{ route('question.delete', $qa->id) }}">
                            ...
                            <form action="{{ route('question.update', $edit->id) }}" method="POST"
                                ...
                                <div class="form-group d-none" id="image_question_id">
                                    ...
                                    <img style="max-width: 50%;" id="question-photo-preview"
                                        src="{{ asset('storage/question/' . $edit->image_question) }}" alt="">
                                    <br>
                                ...
                                <div class="form-actions">
                                    <a class="btn btn-outline-info" href="{{ route('question.view') }}">
        ...
        $(document).ready(function() {
            ...
            function renderExcelValidationResult(response) {
                ...
                if (response.success) {
                    ...
                    $('#uploadExcelButton').prop('disabled', false);
                    $('#uploadWithProblemsButton').hide();
                    $('#forceUploadExcel').val('0');
                } else {
                    ...
                    html += '</ul></div>';
                    $('#uploadExcelButton').prop('disabled', true);
                    $('#uploadWithProblemsButton').show();
                }

                $('#excelValidationResult').html(html);
            }

            $('#questionExcelFile').on('change', function() {
                $('#uploadExcelButton').prop('disabled', true);
                $('#uploadWithProblemsButton').hide();
                $('#forceUploadExcel').val('0');

                if (this.files.length) {
                    $('#excelValidationResult').html('<div class="validation-message validation-neutral mb-0">Status: File selected. Please validate before upload.</div>');
                } else {
                    $('#excelValidationResult').html('<div class="validation-message validation-neutral mb-0">Status: No file validated yet.</div>');
                }
            });

            $('#uploadWithProblemsButton').on('click', function() {
            ...
            $('#validateExcelButton').on('click', function() {
                let fileInput = document.getElementById('questionExcelFile');

                if (!fileInput.files.length) {
                    $('#excelValidationResult').html('<div class="validation-message validation-warning mb-0">Status: No file selected. Please choose an Excel file first.</div>');
                    $('#uploadExcelButton').prop('disabled', true);
                    $('#uploadWithProblemsButton').hide();
                    $('#forceUploadExcel').val('0');
                    return;
                ...
                formData.append('_token', $('input[name="_token"]', '#questionUploadForm').val());
                formData.append('question_excel_file', fileInput.files[0]);

                $('#validateExcelButton').prop('disabled', true).html('<i class="fa fa-spinner fa-spin mr-1"></i> Validating...');
                $('#excelValidationResult').html('<div class="validation-message validation-info mb-0">Status: Validating file... Please wait.</div>');
                $('#uploadExcelButton').prop('disabled', true);
                $('#uploadWithProblemsButton').hide();
                $('#forceUploadExcel').val('0');

                $.ajax({
                    url: '{{ route('question.excel.validate') }}',
                    type: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
                    ...
                    complete: function() {
                        $('#validateExcelButton').prop('disabled', false).html('<i class="fa fa-check-circle mr-1"></i> Validate File');
                    }
                });
            });
        });
